...
<div class="carouselck<?php echo $params->get('moduleclass_sfx'); ?> carouselck_wrap <?php echo $params->get('skin'); ?>" id="carouselck_wrap_<?php echo $module->id; ?>"<?php echo $width; ?>>
	...
	foreach ($items as $item) {
		...
		$imgtitle = htmlspecialchars(str_replace("\"", "&quot;", str_replace(">", "&gt;", str_replace("<", "&lt;", $item->imgtitle))));
		$imgcaption = htmlspecialchars(str_replace("\"", "&quot;", str_replace(">", "&gt;", str_replace("<", "&lt;", $item->imgcaption))));
		$datatitle = ($params->get('lightboxcaption', 'caption') != 'caption') ? 'data-title="' . $imgtitle . '::' . $imgcaption . '" ' : 'data-title=""';
		...
		?>
		<div class="carouselck" <?php echo $datarel . $datatitle; ?> data-thumb="<?php echo $item->imgthumb; ?>" data-src="<?php echo $item->imgname; ?>" <?php if ($item->imglink) echo 'data-link="' . $item->imglink . '" data-target="' . $item->imgtarget . '"'; ?>>
